//
// ASCOM.scopefocusServer Local COM Server
//
...
namespace ASCOM.scopefocusServer
{
    public static class Server
    {
        ...
        private static string s_appId = "{9870a437-bcc5-451a-93eb-2bccb4ce3c40}";	// Our AppId
            ...
            foreach (FileInfo fi in d.GetFiles("*.dll"))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "scopefocusServer", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The scopefocusServer was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "scopefocusServer", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "scopefocusServer", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "scopefocusServer", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
        ...
        static void Main(string[] args)
        {
            ...
            // When debugging uncomment MessageBox.Show() to allow us to attach the debugger to the process.
  //          MessageBox.Show("Starting the Server. Click OK to continue.");
